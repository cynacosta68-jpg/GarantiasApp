...
const NUMERICOS = ['cantidad', 'costo', 'descuento', 'costoNeto', 'costoTotal', 'costoLista', 'costoListaTotal', 'ahorro', 'ahorroPct'];
const TEXTOS = ['repuesto', 'descripcion', 'proveedor', 'deposito', 'documento', 'pedido'];

export async function PATCH(req: NextRequest, { params }: Ctx) {
  const body = await req.json();
  const data: Record<string, unknown> = { editadoManual: true };

  if ('fecha' in body) data.fecha = body.fecha ? new Date(body.fecha) : null;
  for (const c of NUMERICOS) if (c in body) data[c] = Number(body[c] ?? 0);
  for (const c of TEXTOS) {
    if (!(c in body)) continue;
    if (c === 'repuesto' && !body[c]) {
      return NextResponse.json({ error: 'Repuesto no puede quedar vacío.' }, { status: 400 });
    }
    data[c] = body[c] === '' ? null : body[c];
  }
