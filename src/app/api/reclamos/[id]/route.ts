...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  for (const campo of EDITABLES) {
    ...
    if (campo === 'fechaR' || campo === 'fechaFc') data[campo] = v ? new Date(v) : null;
    else if (campo === 'valor') data[campo] = Number(v ?? 0);
    else if (campo === 'patente') data[campo] = v ? String(v).toUpperCase() : null;
    else if (campo === 'reclamo' || campo === 'orden') {
      if (!v) return NextResponse.json({ error: `${campo} no puede quedar vacío.` }, { status: 400 });
      data[campo] = String(v);
    } else data[campo] = v === '' ? null : v;
  }

  if (body.datosExtra && typeof body.datosExtra === 'object') data.datosExtra = body.datosExtra;
